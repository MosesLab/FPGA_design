...
* After integrating the design with user's testbench, the user needs to add
  these files (testbench files, toplevel wrapper file) in DDR2_CORE_JAH.prj under
  synth folder.
...
* "DDR2_CORE_JAH.ucf" file is the constraint file for the design. This is used by
...
* "mem_interface_top.ut" file has the options for the Configuration file
  generation i.e. the "DDR2_CORE_JAH.bit" file to run in batch mode.
...
* "icon4_cg.xco", "vio_async_in96_cg.xco", "vio_async_in100_cg.xco",
  ...
  DDR2_CORE_JAH rtl file. In order to generate the EDIF/NGC files, you must
...
Note : When you generate the design using DEBUG_EN option, the above mentioned
       ...
       create_ise.bat files. The DDR2_CORE_JAH rtl file will have the design
...
* At the start of a Chip Scope Analyzer project, all of the signals in
  every core have generic names. "DDR2_CORE_JAH.cdc" is a file that contains
  all the signal names of all cores. Upon importing this file, signal names are
  renamed to the specified names in "DDR2_CORE_JAH.cdc" file. This file will work
...
* Synth folder has the constraint file for Synplify Pro designs
  i.e. the "mem_interface_top_synp.sdc" file, "DDR2_CORE_JAH.prj" file which
  has the design files to be added to the project, "script_synp.tcl" file
  that sets the synthesis options and "DDR2_CORE_JAH.lso" file which is a custom
...
* For example, the design is generated with component name as DDR2_CORE_JAH with
  Target FPGA LX50t-FF1136 and Compatible FPGA LX85t-FF1136.
   ...
   - Change the UCF file name in batch file to the one in compatible_ucf
     folder i.e., change the UCF name (DDR2_CORE_JAH.ucf) in ise_flow.bat and
